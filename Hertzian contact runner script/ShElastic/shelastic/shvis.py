...
def vismesh(xmesh, cmap='viridis', show=False, SphCoord=True, 
    ...
    if SphCoord:
        ...
        xshift = _np.roll(xmesh, _np.round(lons.shape[1]*lonshift/360).astype(int), axis=1)   #changed _np.int to int here and everywhere else in the file
...
def visSHVec(xvec, lmax_plot=None, cmap='viridis', show=True, 
    ...
    if lmax_plot is None:
        lmax_plot = _np.sqrt(xvec.size//3).astype(int) - 1
