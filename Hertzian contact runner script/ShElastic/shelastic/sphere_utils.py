...
def loadCoeffs(mu0, nu0, lmax, shtype, coeff_dir=None, verbose=True):
    mu = 1.; nu = nu0;    #Why is mu=1. and not mu=mu0? mu0 is unused otherwise.
    lJmax = lKmax = lmax; 
...
#Interpolation of radial displacement component u_r onto a spherical harmonic grid 

def Ur_interp(Vp, lmax=30, plot_figure=False):
    #Convert coordinates and adjust them
    ...
    ur = (Vr - 1)
    #Spherical harmonic expansion to approximate u_r as function of latitude and longitude up to l_max with a goodness of fit chi2
    ...
    #Create a 2d interpolating function f_interp, representing u_r over latitude and longitude points on a GLQ grid
    f_interp = RectBivariateSpline(lats_circular[::-1], lons_circular, fpoints[::-1, ], kx=1, ky=1)
    
    #Optionally visualize u_r and interpolation error
    if plot_figure:
        print('Plotting')
        ...
        axs.set_ylim(-90, 90)
        #plt.show()
        #plt.savefig('/net/ag-schwarz/bq_bkraus/Code/ShElastic/examples/newTest_aug24_2/Ur_interp.png')
    return f_interp
