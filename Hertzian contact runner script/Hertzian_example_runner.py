...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, 'ShElastic', 'examples', 'Data_Hertzian_contact')
...
class Axialsymmetric():
    ...
    def update_fullpath(self,customfoldername=False,custompath=False):
        ...
        # Keep outputs directly in the configured savepath (no run-specific subfolder).
        self.fullsavepath = os.path.join(self.savepath, '')

    def new_run(self,number):
        self.fullsavepath=os.path.join(self.savepath, str(number), '')
        os.makedirs(self.fullsavepath, exist_ok=True)
    ...
    def generate_displaced_bead_positions(self, array, pixelshift=False):
        ...
        The displacement field is the analytical Hertzian contact solution,
        derived from the prescribed stress boundary condition sigma_zz.
        '''

        displaced_beads=np.zeros_like(array)   #Takes bead positions as input "array"
        displacement_vectors=np.zeros_like(array)
        if pixelshift:  
            #pixelshift to add a fixed displacement
            print('pixelshift is True')
            for n,p in enumerate(array):
                u=np.array([0,0,2])*self.res
                displacement_vectors[n]=u
                displaced_beads[n]=p+u

        else:
            for n,p in enumerate(array):
                r,th,phi=self.cart_to_sph_coords(p)   #convert to spherical coordinates
                # Evaluate analytical Hertzian displacement at this point
                u_r=self.u_r(th,r)
                u_t=self.u_th(th,r)
                u=self.vector_sph_to_cart([u_r,u_t,0],th,phi)   #convert back to cartesian
                displacement_vectors[n]=u
                displaced_beads[n]=p+u   #Add displacement to bead position

        # Quick sanity check to ensure the displacement field is physically non-zero
        disp_norm=np.linalg.norm(displacement_vectors,axis=1)
        if len(disp_norm)>0:
            print('Displacement field statistics (analytical Hertzian from stress profile):')
            print(f'  min={disp_norm.min():.3e}, mean={disp_norm.mean():.3e}, max={disp_norm.max():.3e} (code units)')
            if np.allclose(disp_norm,0):
                print('  ⚠️  WARNING: All bead displacements are zero!')
                print('     This will produce identical reference and deformed images.')
                print('     Check that contact coefficients were properly computed.')
            else:
                # Verify displacement field is reasonable for the contact problem
                # Contact region should have largest displacements
                contact_edge_theta = np.arcsin(self.a / self.radius)
                r_test_surface = self.radius
                u_r_contact = self.u_r(contact_edge_theta, r_test_surface)
                u_t_contact = self.u_th(contact_edge_theta, r_test_surface)
                u_contact_mag = np.sqrt(u_r_contact**2 + u_t_contact**2)
                print(f'  Analytical displacement magnitude at contact edge (surface): {u_contact_mag:.3e}')
                if u_contact_mag > 0:
                    ratio = disp_norm.max() / u_contact_mag
                    print(f'    (Generated bead displacements span {ratio:.2f}× this magnitude)')
            # [DEBUG] Detailed displacement vector diagnostics per axis
            print('[DEBUG] Bead displacement per-axis statistics:')
            disp_vectors = displacement_vectors  # shape (N, 3)
            for axis_idx, axis_name in enumerate(['x', 'y', 'z']):
                axis_vals = disp_vectors[:, axis_idx]
                print(f'  {axis_name}-component: mean={np.mean(axis_vals):.3e}, std={np.std(axis_vals):.3e}')
            # Check fraction of beads with near-zero displacement
            near_zero_count = np.sum(disp_norm < 1e-6)
            near_zero_frac = near_zero_count / len(disp_norm) if len(disp_norm) > 0 else 0
            print(f'  Fraction with norm < 1e-6: {near_zero_frac:.6f}')
            # Compare to voxel resolution
            res_norm = np.linalg.norm(self.res)
            print(f'  Voxel size norm: {res_norm:.3e},  Max displacement / voxel norm: {disp_norm.max() / (res_norm + 1e-10):.2f}×')
    ...
    def generate_image_pair(self,boxsize=1.2,beaddensity=0.001,beaddiameter=0.03,
        ...
        if not variablesonly:
            ...
            # Keep reference image tied to undeformed bead positions.
            self.img_ref=self.generate_image_data(reference_bead_positions,noise_amp=noise_amp,disp_noise_amp=0)
            # Apply analytic bead displacement field to displaced positions, plus optional deformation noise.
            self.img_disp=self.generate_image_data(displaced_bead_positions,noise_amp=noise_amp,disp_noise_amp=disp_noise_amp)
            
            # [DEBUG] Verify physical displacement signal in rendered images
            print('[DEBUG] Rendered image difference analysis (before normalization):')
            img_diff_raw = self.img_disp.astype(np.float64) - self.img_ref.astype(np.float64)
            rms_diff = np.sqrt(np.mean(img_diff_raw**2))
            mae_diff = np.mean(np.abs(img_diff_raw))
            max_abs_diff = np.max(np.abs(img_diff_raw))
            print(f'  RMS(deformed - reference): {rms_diff:.3e}')
            print(f'  MAE(deformed - reference): {mae_diff:.3e}')
            print(f'  Max |difference|: {max_abs_diff:.3e}')
            # Pearson correlation (with safety guard)
            ref_flat = self.img_ref.flatten().astype(np.float64)
            disp_flat = self.img_disp.flatten().astype(np.float64)
            ref_std = np.std(ref_flat)
            disp_std = np.std(disp_flat)
            if ref_std > 1e-10 and disp_std > 1e-10:
                pearson_r = np.corrcoef(ref_flat, disp_flat)[0, 1]
                print(f'  Pearson correlation: {pearson_r:.6f}')
            else:
                print(f'  Pearson correlation: undefined (ref_std={ref_std:.3e}, disp_std={disp_std:.3e})')
            # Summary verdict
            if rms_diff < 0.5:
                print(f'  ⚠️  WARNING: RMS difference is very small ({rms_diff:.3e}) — displacement signal may be missing!')
            ...
            # [DEBUG] Normalization safety checks
            print('[DEBUG] Normalization safety diagnostics:')
            print(f'  I0max (peak intensity): {I0max:.3e}')
            ref_nonfinite = np.sum(~np.isfinite(self.img_ref))
            disp_nonfinite = np.sum(~np.isfinite(self.img_disp))
            print(f'  Non-finite voxel counts: img_ref={ref_nonfinite}, img_disp={disp_nonfinite}')
            if I0max <= 0:
                print(f'  ⚠️  CRITICAL: I0max <= 0 — normalization division by zero will produce NaN/Inf!')
            if ref_nonfinite > 0 or disp_nonfinite > 0:
                print(f'  ⚠️  WARNING: Non-finite values present — uint16 cast may produce invalid data!')
            ...
            if savemat:   #Save as matlab file in a dictionary so it appears as "x" in Matlab
    ...
    #Then, analyze the data in Matlab and get results labeled accordingly.
...
beaddiameter = 0.2 / microparticle_radius_meter  # Bead diameter in code units, standard: (200nm) / (12μm radius) ≈ 0.0167
...
os.makedirs(t_Hertzian_contact.fullsavepath, exist_ok=True)
...
# Configure resolution/boxsize for downstream steps.
t_Hertzian_contact.res = res
t_Hertzian_contact.boxsize = boxsize

# Generate synthetic image pair when volume method is requested and FIDVC outputs
# are not yet available. This must be independent of surface_method.
if volume_method:
    ...
    if os.path.exists(fidvc_mesh) and os.path.exists(fidvc_u):
        ...
        print('FIDVC .mat files not found — generating synthetic image pair now...')
        print('[DIAGNOSTIC] Verifying analytical displacement field for Hertzian contact...')
        # Diagnostic: evaluate displacement at sphere surface to verify analytical solution
        theta_test = np.arcsin(t_Hertzian_contact.a / t_Hertzian_contact.radius) / 2  # Test point inside contact
        ur_test = t_Hertzian_contact.u_r(theta_test)
        ut_test = t_Hertzian_contact.u_th(theta_test)
        print(f'  At theta={np.degrees(theta_test):.1f}°: u_r={ur_test:.3e}, u_theta={ut_test:.3e}')
        # Diagnostic: stress field
        sigma_zz_test = t_Hertzian_contact.sigma_zz(theta_test)
        print(f'  Corresponding sigma_zz(analytical)={sigma_zz_test:.3e} Pa')
        
        print('  Generating image pair using analytical Hertzian displacement field...')
    ...
    # After setup/generation, report whether synthetic image-pair files exist.
    try:
        outdir = t_Hertzian_contact.fullsavepath
        ref_candidates = [f for f in os.listdir(outdir) if f.startswith('00Referencepicture') and f.endswith('.mat')]
        def_candidates = [f for f in os.listdir(outdir) if f.startswith('01Deformedpicture') and f.endswith('.mat')]
        if ref_candidates and def_candidates:
            print('Synthetic image pair is available in output directory:')
            print(f'  Reference: {ref_candidates[0]}')
            print(f'  Deformed : {def_candidates[0]}')
            print(f'  Path     : {outdir}')
        else:
            print('WARNING: Synthetic image pair files were not found after setup.')
            print(f'  Checked path: {outdir}')
    except Exception as _img_check_err:
        print(f'WARNING: Could not verify synthetic image-pair files: {_img_check_err}')
...
if surface_method:
    ...
    print('\n  Getting analytical traction on GLQ grid')
...
if volume_method:
    ...
    # Ensure material properties are set correctly for traction calculation
    # This is critical: calc_all_derivatives uses self.G to compute stresses
    mu0 = lame_mu(E0, nu0)
    t_Hertzian_contact.mu = mu0
    t_Hertzian_contact.G = mu0
    print(f'[VOLUME] Set shear modulus: G = mu = {mu0} Pa')

    # If FIDVC displacement outputs are missing, stop after image-pair generation
    # with a clear message rather than failing inside load_prep_FIDVC.
    fidvc_mesh = os.path.join(t_Hertzian_contact.fullsavepath, 'mesh_profile.mat')
    fidvc_u = os.path.join(t_Hertzian_contact.fullsavepath, 'u_profile.mat')
    if not (os.path.isfile(fidvc_mesh) and os.path.isfile(fidvc_u)):
        outdir = t_Hertzian_contact.fullsavepath
        ref_candidates = [f for f in os.listdir(outdir) if f.startswith('00Referencepicture') and f.endswith('.mat')]
        def_candidates = [f for f in os.listdir(outdir) if f.startswith('01Deformedpicture') and f.endswith('.mat')]
        print('FIDVC .mat files are missing; cannot run volume traction recovery yet.')
        print(f'  Missing: {fidvc_u}')
        print(f'  Missing: {fidvc_mesh}')
        if ref_candidates and def_candidates:
            print('Image pair was generated successfully:')
            print(f'  Reference: {ref_candidates[0]}')
            print(f'  Deformed : {def_candidates[0]}')
            print(f'  Path     : {outdir}')
        else:
            print('WARNING: No generated image-pair files were found in the output directory.')
            print(f'  Checked path: {outdir}')
        print('Next step: Run FIDVC (MATLAB) on generated image pair to produce u_profile.mat and mesh_profile.mat, then rerun this script.')
        sys.exit(0)
...
# =====================================================================
# SURFACE METHOD - OPTIMIZATION SETUP
# =====================================================================
...
if surface_method:
    ...
    # Standard zero initialization for Fibonacci
